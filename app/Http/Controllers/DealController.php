...
class DealController extends Controller
{
    ...
    public function index()
    {
        if (\Auth::user()->type == 'company' || \Auth::user()->type == 'employee' || \Auth::user()->type == 'client') {
            ...
            if (!empty($pipeline)) {
                ...
                $getLabelsData = Deal::getLabelsData();

                return view('deal.index', compact('pipelines', 'pipeline', 'cnt_deal', 'getLabelsData'));
            } else {
    ...
    public function productEdit($id)
    {
        ...
        $products = Item::where('created_by', '=', \Auth::user()->creatorId())->whereNOTIn('id', !empty($deal->products) ? explode(',', $deal->products) : [])->get()->pluck('name', 'id');
    ...
    public function deal_list()
    {
        ...
        $deals       = Deal::where('created_by', '=', $usr->creatorId())->where('pipeline_id', '=', $pipeline->id)->with('stage')->get();
        ...
        // Deals
        if ($usr->type == 'client') {
            $deals = Deal::select('deals.*')->with('stage')->join('client_deals', 'client_deals.deal_id', '=', 'deals.id')->where('client_deals.client_id', '=', $usr->id)->orderBy('deals.order')->get();
        } else {
            $deals = Deal::select('deals.*')->with('stage')->join('user_deals', 'user_deals.deal_id', '=', 'deals.id')->where('user_deals.user_id', '=', $usr->id)->orderBy('deals.order')->get();
        }
        $defualtView         = new UserDefualtView();
        $defualtView->route  = \Request::route()->getName();
        $defualtView->module = 'deal';
    ...
    public function import(Request $request)
    {
        ...
        for ($i = 1; $i <= count($deals) - 1; $i++) {
            ...
            $dealData->pipeline_id            =    $deal[1];
            $dealData->stage_id         =     $deal[2];
            $dealData->order        =     0;
            ...
            if (!empty($dealData)) {
                $leaduser                   =    new UserDeal();
                $leaduser->user_id          =    \Auth::user()->creatorId();
                $leaduser->deal_id      =    $dealData->id;
                $leaduser->save();
            }
        }
